...
$query_temp = "CREATE OR REPLACE VIEW vse_ure AS
SELECT SUM(stevilo_ur) as ure, tk_idsodelujoci
FROM projektopia.projekt_sodelujoci
GROUP BY tk_idsodelujoci;";

$link -> query($query_temp);


$FLAG_NOBEN_DEV = FALSE;
if (isset($_POST['_METHOD'])) {

    if ($_POST['_METHOD'] == 'POST_SOD_SPEC') {
        $query_get_sod = "SELECT  ime, priimek, ure, id_sodelujoci
FROM projektopia.sodelujoci, vse_ure
WHERE id_sodelujoci = tk_idsodelujoci
AND priimek LIKE '%" . $_POST['iskanje'] . "%';";
    }
}else{
        $query_get_sod = "
SELECT  ime, priimek, ure, id_sodelujoci
FROM projektopia.sodelujoci, vse_ure
WHERE id_sodelujoci = tk_idsodelujoci;";
}

$RS_get_sod = $link ->query($query_get_sod);
if(mysqli_num_rows($RS_get_sod) == 0){
    $FLAG_NOBEN_DEV = TRUE;
}
            ...
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">
                        Brskanje med developeri
                        <small>za podrobno iskanje vnesite ključno besedo (poizvedovanje po PRIIMKu)</small>
                    </h1>
                    <form action="admin_sodelujoci.php" method="post" role="form">
                        <div class="col-lg-6">
                            <label for="iskanje">Iskanje po developerih</label>
                        ...
                        <div class="col-lg-2">
                            ...
                            <input type="hidden" name="_METHOD" value="POST_SOD_SPEC"/>
                ...
                <div class="col-lg-12">
                    <?php
                    if($FLAG_NOBEN_DEV){
                        echo "Najden ni bil noben taki developer ki bi usrezal ključni besedi :(";
                    }else{
                        while($row = $RS_get_sod ->fetch_array()){
                            echo "<div class='col-lg-3'>";
                            echo "<div class=\"panel panel-default\">";
                            echo "<div class=\"panel-body\">";
                            echo strtoupper($row['ime']) . " " . strtoupper($row['priimek']) . " &#8594;" . $row['ure'] ." ur";
                            echo "</div>";
                            echo "</div>";
                            echo "</div>";
                            echo "<div class='col-lg-1 text-left'>";
                            echo "<a href='sodelujoci.php?id=" . $row['id_sodelujoci'] . "' class='btn btn-warning'>Poglej</a>";
                            echo "</div>";
